		...
		try {
			...
			for (Map.Entry<Produit, Integer> entry : obj.get_produitsvendus().entrySet()) {
				PreparedStatement ps2 = conn
						.prepareStatement("INSERT INTO produit_facture(id_facture,id_produit,quant) VALUES (?,?,?);");
			...
			while (rs.next()) {
				if (facture == null) {
					facture = new Facture(rs.getInt("f.id_facture"), this.clientDAO.find(rs.getInt("f.id_client")),
							this.utilisateurDAO.find(rs.getInt("f.id_utilisateur")), new HashMap<>());
				}
				facture.get_produitsvendus().put(this.produitDAO.find(rs.getInt("pf.id_produit")),
						rs.getInt("pf.quant"));
			}
			ps.close();
			return facture;
